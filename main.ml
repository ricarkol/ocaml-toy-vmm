...
let main () =
  ...
  let _ =
    ioctl vm_fd _KVM_SET_TSS_ADDR
      (ptr_of_raw_address (Nativeint.of_int 0xffffd000))
  in
  (*
...
*)
  ...
  let _ =
    ioctl (int_of_file_descr kvm_fd) _KVM_GET_SUPPORTED_CPUID
      Ctypes.(to_voidp (addr kvm_cpuid))
  in
  let _ =
    ioctl (int_of_file_descr kvm_fd) _KVM_SET_CPUID2
      Ctypes.(to_voidp (addr kvm_cpuid))
  in
